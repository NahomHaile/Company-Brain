...
function ResultPanel({
  ...
  const capturedAt = new Date(cached.captured_at).toLocaleTimeString("en-US", {
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-center gap-2">
          {run.source === "cached" ? (
            <>
              <HonestyBadge kind="cached" suffix={`fetched ${capturedAt}`} />
              {cached.captured_from_live_run ? null : <HonestyBadge kind="synthetic" />}
            </>
          ) : null}
        </div>
      ...
      <CardContent className="flex flex-col gap-5">
        <div className="flex flex-wrap gap-x-8 gap-y-3">
          ...
          <Stat value={pendingFlags} label="claims flagged" tone={pendingFlags ? "warn" : undefined} />
